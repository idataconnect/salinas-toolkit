...
public class TWindow extends TWidget {
    ...
    /**
     * If true, this window is minimized.
     */
    ...
    // For iconized restore
    private int iconRestoreWidth;
    private int iconRestoreHeight;
    private int iconRestoreX;
    private int iconRestoreY;
    ...
    /**
     * The last time a mouse button was pressed, for double-click detection.
     */
    private long lastClickTime = 0;

    /**
     * If true, this window is minimized AND inside the automatic dock.
     */
    ...
    protected boolean mouseOnClose() {
        if (minimized || (flags & NOCLOSEBOX) != 0) {
            return false;
        }
        if ((mouse != null)
    ...
    protected boolean mouseOnMaximize() {
        if (minimized || (flags & NOZOOMBOX) != 0) {
            return false;
        }
        if ((mouse != null)
            && !isModal()
    ...
    /**
     * Returns true if the mouse is currently on the minimize button.
     *
     * @return true if the mouse is currently on the minimize button
     */
    protected boolean mouseOnMinimize() {
        if (minimized || (flags & NOZOOMBOX) != 0) {
            return false;
        }
        if ((mouse != null)
            && !isModal()
            && (mouse.getAbsoluteY() == getY())
            && (mouse.getAbsoluteX() >= getX() + getWidth() - 7)
            && (mouse.getAbsoluteX() <= getX() + getWidth() - 5)
        ) {
            return true;
        }
        return false;
    ...
    protected void onFocus() {
        if (!(this instanceof TDesktop)) {
            application.enableMenuItem(TMenu.MID_WINDOW_CLOSE);
        }
    }
    ...
    protected void onUnfocus() {
        if (!(this instanceof TDesktop)) {
            application.disableMenuItem(TMenu.MID_WINDOW_CLOSE);
        }
    }
    ...
    public void onMouseDown(final TMouseEvent mouse) {
        ...
        if (minimized
            && (mouse.getType() == TMouseEvent.Type.MOUSE_DOUBLE_CLICK
                || (mouse.isMouse1() && (System.currentTimeMillis() - lastClickTime < 500)))
        ) {
            restore();
            return;
        }
    ...
    public void onMouseUp(final TMouseEvent mouse) {
        ...
        if ((inWindowMove) && (mouse.isMouse1())) {
            ...
            if (minimized) {
                // If the user drops the icon back in the dock area, re-dock it!
    ...
    public void onMouseMotion(final TMouseEvent mouse) {
        ...
        if (inWindowMove) {
            ...
            if (minimized) {
                // Moving a minimized icon undocks it.
                docked = false;
            }
            ...
            // Allow going all the way to the bottom
            int h = getScreen().getHeight();
            if (getY() >= h) {
                setY(h - 1);
            }
    ...
    public void draw() {
        ...
        if ((title != null) && (title.length() > 0)) {
            ...
            if (minimized) {
                // Show as: TITLE [N]
                displayTitle = String.format("%s [%d]", title, getWindowNumber());
                if (displayTitle.length() > getWidth() - 2) {
                    displayTitle = displayTitle.substring(0, getWidth() - 2);
                }
            }

            int titleLength = StringUtils.width(displayTitle);
            int titleLeft = (getWidth() - titleLength - 2) / 2;
            if (minimized) {
                titleLeft = 1; // Left-aligned in icon
            }
            putCharXY(titleLeft, 0, ' ', border);
            putStringXY(titleLeft + 1, 0, displayTitle, border);
        ...
        if (isActive() && !minimized) {
            ...
            if (!isModal() && ((flags & NOZOOMBOX) == 0)) {
                putCharXY(getWidth() - 4, 0, lBracket, border);
                putCharXY(getWidth() - 2, 0, rBracket, border);
                if (mouseOnMaximize() && mouse.isMouse1()) {
                    putCharXY(getWidth() - 3, 0, GraphicsChars.CP437[0x0F],
                        getBorderControls());
                } else {
                    if (maximized) {
                        putCharXY(getWidth() - 3, 0, GraphicsChars.CP437[0x12],
                            getBorderControls());
                    } else {
                        putCharXY(getWidth() - 3, 0, GraphicsChars.UPARROW,
                            getBorderControls());
                    }
                }

                // Draw the minimize button
                putCharXY(getWidth() - 7, 0, lBracket, border);
                putCharXY(getWidth() - 5, 0, rBracket, border);
                if (mouseOnMinimize() && mouse.isMouse1()) {
                    putCharXY(getWidth() - 6, 0, GraphicsChars.CP437[0x0F],
                        getBorderControls());
                } else {
                    putCharXY(getWidth() - 6, 0, GraphicsChars.DOWNARROW,
                        getBorderControls());
                }
            }
    ...
    public String getMouseStyle() {
        return System.getProperty("com.idataconnect.salinas.toolkit.Swing.mouseStyle", "default");
    }
    ...
    /**
     * Restore (unmaximize or unminimize) window.
     */
    public void restore() {
        if (maximized) {
            ...
            setWidth(iconRestoreWidth);
            setHeight(iconRestoreHeight);
            setX(iconRestoreX);
            setY(iconRestoreY);
            minimized = false;
            docked = false;
    ...
    public void minimize() {
        ...
        iconRestoreWidth = getWidth();
        iconRestoreHeight = getHeight();
        iconRestoreX = getX();
        iconRestoreY = getY();
        ...
        // TApplication will handle positioning in its draw loop or 
        // specialized method.
        application.onWindowMinimize(this);
    ...
    /**
     * Get the docked state.  If true, the window is minimized and in the 
     * dock.
     *
     * @return true if minimized and in the dock
     */
    public final boolean isDocked() {
        return (minimized && docked);
    }
     ...
     */
    public final void setWindowNumber(final int windowNumber) {
     ...
     * null to use the value from com.idataconnect.salinas.toolkit.TWindow.borderStyleForeground.
     */
    public void setBorderStyleForeground(final String borderStyle) {
        if (borderStyle == null) {
            String style = System.getProperty("com.idataconnect.salinas.toolkit.TWindow.borderStyleForeground",
                "double");
     ...
     * @param borderStyle the border style string, one of: "default", "none",
     * "single", "double", "singleVdoubleH", "singleHdoubleV", or "round"; or
     * null to use the value from com.idataconnect.salinas.toolkit.TWindow.borderStyleModal.
     */
    public void setBorderStyleModal(final String borderStyle) {
        if (borderStyle == null) {
            String style = System.getProperty("com.idataconnect.salinas.toolkit.TWindow.borderStyleModal",
                "double");
     ...
     * @param borderStyle the border style string, one of: "default", "none",
     * "single", "double", "singleVdoubleH", "singleHdoubleV", or "round"; or
     * null to use the value from com.idataconnect.salinas.toolkit.TWindow.borderStyleInactive.
     */
    public void setBorderStyleInactive(final String borderStyle) {
        if (borderStyle == null) {
            String style = System.getProperty("com.idataconnect.salinas.toolkit.TWindow.borderStyleInactive",
                "single");
     ...
     * @param borderStyle the border style string, one of: "default", "none",
     * "single", "double", "singleVdoubleH", "singleHdoubleV", or "round"; or
     * null to use the value from com.idataconnect.salinas.toolkit.TWindow.borderStyleMoving.
     */
    public void setBorderStyleMoving(final String borderStyle) {
        if (borderStyle == null) {
            String style = System.getProperty("com.idataconnect.salinas.toolkit.TWindow.borderStyleMoving",
                "single");
