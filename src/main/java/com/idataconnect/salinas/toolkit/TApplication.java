...
import com.idataconnect.salinas.toolkit.backend.ECMA48Backend;
import com.idataconnect.salinas.toolkit.backend.MultiBackend;
import com.idataconnect.salinas.toolkit.backend.Screen;
import com.idataconnect.salinas.toolkit.backend.SwingBackend;
import com.idataconnect.salinas.toolkit.backend.SwingTerminal;
import com.idataconnect.salinas.toolkit.backend.TWindowBackend;
...
import com.idataconnect.salinas.toolkit.bits.ColorTheme;
import com.idataconnect.salinas.toolkit.bits.StringUtils;
import com.idataconnect.salinas.toolkit.effect.Effect;
import com.idataconnect.salinas.toolkit.effect.TextCursorGlintEffect;
import com.idataconnect.salinas.toolkit.effect.WindowBurnInEffect;
import com.idataconnect.salinas.toolkit.effect.WindowBurnOutEffect;
import com.idataconnect.salinas.toolkit.effect.WindowFadeInEffect;
import com.idataconnect.salinas.toolkit.effect.WindowFadeOutEffect;
import com.idataconnect.salinas.toolkit.effect.WindowWipeInEffect;
import com.idataconnect.salinas.toolkit.effect.WindowWipeOutEffect;
...
import com.idataconnect.salinas.toolkit.help.HelpFile;
...
import com.idataconnect.salinas.toolkit.tackboard.Tackboard;
...
public class TApplication implements Runnable {
    ...
    private class ScreenHandler implements Runnable {
        ...
        /**
         * Get the last frame render time.
         *
    ...
    private void TApplicationImpl() {
        ...
        if (System.getProperty("com.idataconnect.salinas.toolkit.textMouse", "true").equals("false")) {
            textMouse = false;
        }

        // Hide mouse when typing option
        if (System.getProperty("com.idataconnect.salinas.toolkit.hideMouseWhenTyping",
                "false").equals("true")) {
        ...
        if (System.getProperty("com.idataconnect.salinas.toolkit.hideStatusBar",
                "false").equals("true")) {
            hideStatusBar = true;
        }

        // Hide menu bar option
        if (System.getProperty("com.idataconnect.salinas.toolkit.hideMenuBar", "false").equals("true")) {
            hideMenuBar = true;
        }

        // Translucent windows (!) option
        if (System.getProperty("com.idataconnect.salinas.toolkit.translucence", "true").equals("false")) {
            translucence = false;
        }

        // Overall animations
        if (System.getProperty("com.idataconnect.salinas.toolkit.animations", "true").equals("false")) {
            animationsEnabled = false;
        }

        theme           = new ColorTheme();
        desktopTop      = (hideMenuBar ? 0 : 1);
        desktopBottom   = getScreen().getHeight();
        ...
        if (animationsEnabled) {
            ...
            addTimer(1000 / ANIMATION_FPS, true,
                new TAction() {
                    public void DO() {
                        doRepaint();
                    }
            ...
            try {
                String milliStr = System.getProperty("com.idataconnect.salinas.toolkit.blinkMillis",
                    "500");
    ...
    protected boolean onKeypress(final TKeypressEvent keypress) {
        ...
        // Process menu and window application shortcuts
        if (!keypress.getKey().isFnKey()
            ...
            // Process Ctrl-Alt-1, Ctrl-Alt-2, etc. window hotkeys
            char ch = (char) keypress.getKey().getChar();
            if (keypress.getKey().isCtrl() && ch >= '1' && ch <= '9') {
                int number = ch - '0';
            ...
            if (keypress.getKey().isCtrl() == false) {
    ...
    private void metaHandleEvent(final TInputEvent event) {
        ...
        if (event instanceof TResizeEvent) {
            ...
            synchronized (getScreen()) {
                if ((resize.getWidth() != getScreen().getWidth())
    ...
    public int getFramesPerSecond() {
        if (screenHandler != null) {
            return screenHandler.getFramesPerSecond();
        }
        return 0;
    }
    ...
    public long getLastFrameTime() {
        if (screenHandler != null) {
            return screenHandler.getLastFrameTime();
        }
        return 0;
    }
    ...
    public void setHideStatusBar(final boolean hideStatusBar) {
        ...
        desktopBottom = getScreen().getHeight() - 3 + (hideStatusBar ?
            1 : 0);
        if (desktop != null) {
            desktop.setDimensions(0, desktopTop, getScreen().getWidth(),
                (desktopBottom - desktopTop));
        ...
        if (customMousePointer == null) {
            ...
            if (getScreen() instanceof SwingTerminal) {
                ...
                terminal.setMouseStyle(System.getProperty(
                    "com.idataconnect.salinas.toolkit.Swing.mouseStyle", "default"));
            }
            return;
        }
    ...
    private void setMouseState(final TMouseEvent mouse) {
        ...
        String mouseStyle = System.getProperty("com.idataconnect.salinas.toolkit.Swing.mouseStyle",
            "default");
    ...
    private void drawTranslucentWindow(final Screen screen,
        ...
        if (window instanceof com.idataconnect.salinas.toolkit.effect.WindowTransitionEffect.DummyWindow) {
     ...
     */
    public void onWindowMinimize(final TWindow window) {
        // Auto-arrange minimized windows in the dock
        int x = 0;
    ...
    private void drawAll() {
        ...
        // --- LAYER 0: Status Bar & Dock Area (south-anchored) ---
        ...
        // --- LAYER 3: Minimized windows (ALWAYS on top of regular windows) ---
        if (hideStatusBar == false) {
            for (TWindow window: sorted) {
                if (window.isShown() && window.isMinimized()) {
                    getScreen().resetClipping();
        ...
        // --- LAYER 4: Menus (Always on top) ---
        if (hideMenuBar == false) {
            // Draw the blank menubar line
            getScreen().resetClipping();
    ...
    public final void closeWindow(final TWindow window) {
        ...
        if (!window.disableCloseEffect() && animationsEnabled) {
            String windowCloseEffect = System.getProperty("com.idataconnect.salinas.toolkit.effect.windowClose",
                "none").toLowerCase();
    ...
    final void addWindowToApplication(final TWindow window) {
        // Assign a window number
        synchronized (windows) {
            int maxNumber = 0;
            for (TWindow w : windows) {
                if (w.getWindowNumber() > maxNumber) {
                    maxNumber = w.getWindowNumber();
                }
            }
            window.setWindowNumber(maxNumber + 1);
        }
        ...
        if (!window.disableOpenEffect() && animationsEnabled) {
            ...
            String windowOpenEffect = System.getProperty(
                "com.idataconnect.salinas.toolkit.effect.windowOpen", "none").toLowerCase();
    ...
    private void tileWindows() {
        synchronized (windows) {
            ...
            com.idataconnect.salinas.toolkit.bits.WidgetUtils.tileWidgets(windows, getScreen().getWidth(),
    ...
    private void checkSwitchFocus(final TMouseEvent mouse) {
        ...
        if (((focusFollowsMouse == true)
            ...
            synchronized (windows) {
                ...
                // --- Pass 1: Minimized windows (ALWAYS on top) ---
                for (TWindow window: windows) {
